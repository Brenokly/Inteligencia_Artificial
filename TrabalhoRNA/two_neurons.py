...
def activation_func(x): #função de ativação sigmoide
    return 1 / (1 + np.exp(-x))

def derivation_func(x): #derivada da função de ativação sigmoide
    return x * (1 - x)

# def activation_func(x): #função de ativação tangente hiperbólica
#     return np.tanh(x)
...
hidden_layer_neurons = 2 # quantidade de neurônios na camada oculta
...
for _ in range(8500):  # Número de épocas (iterações de treinamento)
